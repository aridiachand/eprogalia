...
@section('content')
                            ...
                            <table id="zero_config_pengajuan_master_barang" class="table table-striped table-bordered">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Nama Barang (From Logistik)</th>
                                        <th>Nama Barang (From Inventory)</th>
    ...
    @includeIf('pengajuan-master-barang.form-pmb-check-inventory')
@endsection
    ...
    <script>
        ...
        $(function() {
            table = $("#zero_config_pengajuan_master_barang").DataTable({
                ...
                autoWidth: false,
                ajax: {
                    url: '{{ route('pengajuan-master-barang.datareqinv') }}',
                },
                columns: [{
                        data: 'DT_RowIndex',
                        searchable: false,
        ...
        $('#modal-form-pmb-check-inventory .suggestfrominventory').select2({
        placeholder: 'Select Barang',
        ajax: {
            url: '{{ route('autocomplete.search2') }}',
            dataType: 'json',
        ...
        function invForm(url) {
            $('#modal-form-pmb-check-inventory').modal('show');
            $('#modal-form-pmb-check-inventory .modal-title').text('Check Master');

            $.get(url)
            .done((response) => {
                console.log(response);
                    $("#modal-form-pmb-check-inventory [name=nama_barang_permintaan]").val(response.suggest_nama_barang);
                    $('#modal-form-pmb-check-inventory #id').val(response.id);
                })
                .fail((errors) => {
                    console.log(errors);
                    alert('Tidak dapat menampilkan data');
